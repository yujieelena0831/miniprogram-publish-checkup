...
- 只读取用户主动提供的小程序文件夹或 ZIP，以及完成判断所需的最少项目文件；
- 默认只运行仓库内三个 Python 标准库脚本，不自动安装依赖；
- 只有在核验会变化的平台规则或用户指定的生产依赖时，才进行必要的只读联网检查；
- 不会为了检查而把项目源码、用户内容、压缩包或凭证发送给第三方；
- 不读取、打印、保存或传输 AppSecret、API Key、上传密钥、Cookie、密码或私钥；发现疑似凭证时只报告文件位置和类型，并对值进行遮蔽；
- 文件修改、安装依赖、预览、上传、部署、提交审核、正式发布以及生产环境变更，均需要用户对具体动作明确授权；
- 报告遵循数据最小化原则，不复制与问题无关的私密内容。

完整约束见 [`SKILL.md`](SKILL.md) 的 `Permissions, Data Handling, And Safety`。
